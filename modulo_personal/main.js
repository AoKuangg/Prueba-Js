...
const getUserAll = async()=>{
    config.method = "GET";
    const users = await (await fetch ("http://localhost:4001/personal",config)).json();
    if (users.length == 0) {
...
const postUser = async (data)=>{
    ...
    let res = await ( await fetch("http://localhost:4001/personal",config)).json();
}
//Funcion asincrona para "Actualizar" los datos que se encuentran en el Json-server
const putUser = async(data)=>{
    config.method = "PUT";
    config.body = JSON.stringify(data);
    let res = await ( await fetch(`http://localhost:4001/personal/${data.id}`,config)).json();
}
...
const searchUser = async(data)=>{
    config.method = "GET";
    let res = await ( await fetch(`http://localhost:4001/personal/${Object.values(data).join("")}`,config)).json();
}
  ...
  //Se usa un forEach para todos los users dentro de data para crear el div y los span requeridos para adecuarlos en el html
    users.forEach((data) => {
      ...
      itemUser.innerHTML = `
          ...
          <span class="date">${data.apellidos}</span>
