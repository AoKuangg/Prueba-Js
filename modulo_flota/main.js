...
const getUserAll = async()=>{
    config.method = "GET";
    const users = await (await fetch ("http://localhost:4001/flota",config)).json();
    if (users.length == 0) {
...
const postUser = async (data)=>{
    ...
    let res = await ( await fetch("http://localhost:4001/flota",config)).json();
}
//Funcion asincrona para "Actualizar" los datos que se encuentran en el Json-server
const putUser = async(data)=>{
    config.method = "PUT";
    config.body = JSON.stringify(data);
    let res = await ( await fetch(`http://localhost:4001/flota/${data.id}`,config)).json();
...
const searchUser = async(data)=>{
    config.method = "GET";
    let res = await ( await fetch(`http://localhost:4001/flota/${Object.values(data).join("")}`,config)).json();
}
...
function renderUsers(users) {
    ...
    users.forEach((data) => {
      ...
      itemUser.innerHTML = `
          <span>${data.id}</span>
          <span class="number">${data.pasajeros}</span>
